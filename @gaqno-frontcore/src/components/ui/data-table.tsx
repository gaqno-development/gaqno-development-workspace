...
export interface DataTableProps<TData, TValue> {
  ...
  renderFilterSheetContent?: React.ReactNode | ((form: ReturnType<typeof useForm<Record<string, unknown>>>) => React.ReactNode)
...
export function DataTable<TData, TValue>({
  ...
  const isEmptySearch = !isEmpty && hasActiveFilters && filteredRows.length === 0
        ...
        <Card className="border-border bg-card text-card-foreground">
          {cardTitle != null && (
            <CardHeader>
              ...
              {cardDescription != null && <CardDescription>{cardDescription}</CardDescription>}
            </CardHeader>
          )}
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <TableHead key={header.id} className="border-border bg-muted/50 text-foreground">
  ...
  const filterSheetContent =
    typeof renderFilterSheetContent === 'function'
      ? renderFilterSheetContent(filterForm as ReturnType<typeof useForm<Record<string, unknown>>>)
      : renderFilterSheetContent
  ...
  const tableContent = (
    <div className="w-full space-y-4">
          ...
          <div className="flex flex-1 flex-wrap items-center gap-2 py-2">
            ...
            {renderFilterSheetContent != null && (
              <Sheet open={filterSheetOpen} onOpenChange={setFilterSheetOpen}>
                ...
                >
                  <SheetHeader>
                    <SheetTitle className="text-foreground">{filterSheetTitle}</SheetTitle>
                  </SheetHeader>
                  {typeof renderFilterSheetContent === 'function' ? (
                    <Form {...filterForm}>
                      <form
      ...
      {!showEmptyState && !showEmptySearchState && showPagination && !infiniteScroll && filteredRows.length > 0 && (
        <DataTablePagination table={table} />
      )}

      {infiniteScroll && onLoadMore && !showEmptyState && !showEmptySearchState && (
        <div ref={scrollSentinelRef} className="h-4 w-full" aria-hidden />
      )}

      {openOnRowActionType === 'sheet' && selectedRow != null && renderRowDetail && (
        <Sheet open={!!selectedRow} onOpenChange={(open) => !open && handleCloseRowDetail()}>
          <SheetContent
            side="right"
      ...
      {openOnRowActionType === 'dialog' && selectedRow != null && renderRowDetail && (
        <Dialog open={!!selectedRow} onOpenChange={(open) => !open && handleCloseRowDetail()}>
      ...
      {openOnRowActionType === 'popover' && selectedRow != null && renderRowDetail && (
        <Dialog open={popoverOpen} onOpenChange={(open) => !open && handleCloseRowDetail()}>
    ...
    return (
      <Card className="border-border bg-card text-card-foreground shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            {cardTitle != null && <CardTitle className="text-card-foreground">{cardTitle}</CardTitle>}
