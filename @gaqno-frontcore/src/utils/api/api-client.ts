...
const handleRefreshError = () => {
  ...
  const isPublicRoute = currentPath.startsWith('/rpg') ||
                       currentPath === '/' ||
...
export const createAxiosClient = (config: AxiosClientConfig = {}): AxiosInstance => {
  ...
  instance.interceptors.response.use(
    onResponse,
    async (error: AxiosError) => onResponseError(error, onRefreshToken, onRefreshError)
  );
...
const getServiceBaseUrl = (serviceName: string): string => {
  ...
  const envUrls: Record<string, string> = {
    ...
    rpg: getEnvVar('VITE_RPG_SERVICE_URL', 'http://localhost:4007'),
  };
