...
Assim o CI e o app GitHub for Jira rodam no repo correto. Ver [../governance/workspace-workflow.md](../governance/workspace-workflow.md).
...
## 4. JQL: Histórias em Code Review (quando subtasks têm commit)

**Regra de processo:** quando uma história tem todos os subtasks com commit (evidência de desenvolvimento), a história deve estar em **Code Review**.

### Listar histórias em Code Review

```text
project = GAQNO AND issuetype = Story AND status = "Code Review" ORDER BY updated DESC
```

### Listar histórias em Code Review com PR aberta (fila de review)

```text
project = GAQNO AND issuetype = Story AND status = "Code Review" AND development[pullrequests].open > 0 ORDER BY updated DESC
```

### Listar subtasks que têm desenvolvimento (commits/PRs)

Útil para conferir se cada subtask já tem commit com a key na mensagem (ex.: GAQNO-1178):

```text
project = GAQNO AND issuetype = Sub-task AND development[pullrequests].open > 0 ORDER BY parent ASC, key ASC
```

Se o app expor commits no JQL (sintaxe pode variar por instância):

```text
project = GAQNO AND issuetype = Sub-task AND development[commits].count > 0 ORDER BY parent ASC, key ASC
```

### Automatizar "todos os subtasks com commit → história em Code Review"

O JQL sozinho não move a história. Para **mover a história para Code Review quando todos os subtasks tiverem commit**:

1. **Automation for Jira** (Project settings → Automation): criar regra com trigger (ex.: **Sub-task transitioned to Done** ou **Development information added**), condição (ex.: **All sub-tasks of parent have development** ou checar status dos subtasks) e ação **Transition parent issue** → Code Review.
2. Ou **processo manual**: quando os 4 commits (subtasks) estiverem na branch da história, fazer a transição da história para Code Review no Jira.
