...
Este guia descreve a ordem correta para subir apenas o necessário e testar no browser o fluxo event-driven: **Login (SSO)** → **Deal Won no CRM** → BullMQ/Redis → **Receita no Financeiro**.

## 0. Pré-requisitos

- **Docker Desktop** (ou Docker Engine) em execução — necessário para Redis.
...
| 2     | `npm run dev:crm-service`   | 4003  | API Deals + BullMQ producer        |
| 3     | `npm run dev:finance-service` | 4005 | API Transações + BullMQ consumer   |
...
- Parar apenas Redis: `docker compose stop redis`
