...
| `mcp-cloudflare-remote.sh` | Cursor **cloudflare-api** MCP: runs `mcp-remote` with **API token only** (no OAuth). Put the token in `.cursor/cloudflare-api-token` (one line, folder gitignored) or set `CLOUDFLARE_API_TOKEN` in the server `env` in `.cursor/mcp.json`. OAuth + Cursor reload causes `Invalid PKCE code_verifier` and hung secondaries. |
