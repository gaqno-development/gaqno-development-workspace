#!/usr/bin/env node
/**
 * Add Public Hostname (ingress) for n8n.gaqno.com.br to the Cloudflare Tunnel
 * so the tunnel routes traffic to Traefik (localhost:80), which then routes by Host to the n8n container (port 5678).
 *
 * Requires: CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID
 * Optional: CLOUDFLARE_TUNNEL_ID (if not set, uses first tunnel or one named like GAQNO_PROD).
 *
 * Usage:
 *   CLOUDFLARE_API_TOKEN=xxx CLOUDFLARE_ACCOUNT_ID=xxx node scripts/cloudflare-tunnel-add-n8n-route.mjs
 */
...
const N8N_ORIGIN = process.env.N8N_ORIGIN || "http://localhost:80";
...
async function main() {
  ...
  const rest = ingress.filter((r) => r.hostname !== "n8n.gaqno.com.br" && r.service !== "http_status:404" && !r.service?.startsWith("http_status:"));

  const n8nRule = { hostname: "n8n.gaqno.com.br", service: N8N_ORIGIN };
  const newIngress = [...rest, n8nRule, ...catchAll];
  ...
  console.log("Added route: n8n.gaqno.com.br ->", N8N_ORIGIN);
