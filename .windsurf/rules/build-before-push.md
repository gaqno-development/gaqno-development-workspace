...
2. **Use Docker for deployment validation**: When checking that a service will build on Dokploy/CI, build with Docker (e.g. `docker build -f <service>/Dockerfile <service>`) so the same Dockerfile and runtime as production is used; local `npm run build` can pass while the Docker build fails.
