...
2. **Dokploy** – Regenerate the token; update local/dokploy config (e.g. in local `mcp.json`).
...
6. **GitHub Actions** – If you used `COOLIFY_WEBHOOK_URL`, create **`DOKPLOY_WEBHOOK_URL`** in repo secrets (Dokploy deploy webhook) and remove the old secret name.
...
- **Secret scanning**: Keep enabled; consider **Copilot custom patterns** for internal token formats (e.g. Postgres, Dokploy) so more leaks are preventable.
